...
public class Dekanat {
    ...
    public void studTransfer(int id, String title) {
        Student s = null;
        int g = -1;
    ...
    public void studTransfer(String fio, String title) {
        Student s = null;
        int g = -1;
        for (int i = 0; i < NumGroups; i++) {
    ...
    public void studTransfer(Student s,Group g){
        if ((s!=null)&&(g!=null)) {
            s.getGroup().deleteStud(s);
            g.addStudent(s);
        }
    }
    public void badStudentsGetLost() {
        Student s;
        int del=0;
        for (int i=0;i<NumStuds;i++) {
            s=Students[i];
    ...
    public void refresh() {
        try {
            Writer out = new BufferedWriter(new OutputStreamWriter( new FileOutputStream(gr), "UTF-8"));
        ...
        try {
            Writer out = new BufferedWriter(new OutputStreamWriter( new FileOutputStream(st), "UTF-8"));
